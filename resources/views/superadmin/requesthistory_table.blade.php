...
@section('content')
    <div class="container mx-auto px-4 py-6">
        ...
        <h1 class="text-3xl font-bold mb-8 text-gray-800">Request History</h1>
        ...
        <div class="bg-white rounded-lg shadow-md overflow-hidden border border-gray-200">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-indigo-700">
                    <tr>
                        <th class="px-6 py-4 text-left text-sm font-semibold text-white uppercase tracking-wider">Staff ID</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach($requestHistories as $history)
                        <tr class="hover:bg-indigo-50 transition-colors duration-150">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $history->staff_id }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $history->unique_code }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $history->part_number }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $history->part_name }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $history->created_at }}</td>
    ...
    <div class="flex items-center space-x-4">
        ...
        <!-- Modal for Editing Request History -->
            ...
            <div class="bg-white rounded-lg shadow-xl w-full max-w-md mx-4 transform transition-all duration-300">
                <div class="p-6">
                    <div class="flex justify-between items-center mb-4">
                        <h2 class="text-xl font-bold text-gray-800">Edit Request History</h2>
                    ...
                    <form action="" method="POST" id="editForm">
                        ...
                        <div class="mb-6">
                            <label for="created_at" class="block text-sm font-medium text-gray-700 mb-2">Created At</label>
                            <input 
                                type="text" 
                                id="editCreatedAt" 
                                name="created_at" 
