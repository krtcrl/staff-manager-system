                ...
                <div class="w-full lg:w-1/2 flex flex-col">
                    ...
                    <div class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm flex flex-col flex-grow">
                        <div class="flex-grow">
                            ...
                            @if ($showButtons && !$hideButtons)
                                <div class="mb-4 flex space-x-2">
                                    ...
                                    <form action="{{ route('manager.request.approve', $request->unique_code) }}" method="POST">
                                        @csrf
                                        <button type="submit" class="px-3 py-1 bg-green-500 text-white rounded hover:bg-green-600 dark:bg-green-600 dark:hover:bg-green-700">
                                            Approve Request
                                        </button>
                                    </form>

                                    <!-- Reject Button -->
                                    <button id="reject-button" class="px-3 py-1 bg-red-500 text-white rounded hover:bg-red-600 dark:bg-red-600 dark:hover:bg-red-700">
                                        Reject Request
                                    </button>
                                </div>

                                <!-- Rejection Form (Initially Hidden) -->
                                <div id="reject-form" class="hidden bg-gray-100 dark:bg-gray-700 p-4 rounded shadow-md">
                                    <form id="reject-form-submit" action="{{ route('manager.request.reject', $request->unique_code) }}" method="POST">
                                        ...
                                        <button type="submit" class="mt-2 px-3 py-1 bg-red-500 text-white rounded hover:bg-red-600 dark:bg-red-600 dark:hover:bg-red-700">
                                            Submit Rejection
                                        </button>
                                    </form>
                                </div>
                            ...
                            @if (session('success'))
                                <div class="mb-2 p-2 bg-green-100 dark:bg-green-800 border border-green-400 dark:border-green-700 text-green-700 dark:text-green-300 rounded">
                                    {{ session('success') }}
                                </div>
                            @endif

                            @if (session('error'))
                                <div class="mb-2 p-2 bg-red-100 dark:bg-red-800 border border-red-400 dark:border-red-700 text-red-700 dark:text-red-300 rounded">
                                    {{ session('error') }}
                                </div>
                            @endif

                            <div class="space-y-2">
                                <div><span class="font-semibold text-gray-800 dark:text-gray-300">Unique Code:</span> <span class="text-gray-800 dark:text-gray-300">{{ $request->unique_code }}</span></div>
