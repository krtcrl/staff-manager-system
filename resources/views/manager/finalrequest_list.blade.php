...
@section('content')
    ...
    <script>
        ...
        channel.bind("new-finalrequest", function (data) {
            ...
            // Add the new final request to the table
            let newRow = `
                <tr id="finalrequest-row-${finalRequest.unique_code}" class="hover:bg-gray-50 transition-colors">
                    <td class="py-2 px-3 text-sm text-gray-700"></td> <!-- Empty, will be updated -->
                    <td class="py-2 px-3 text-sm text-blue-500 hover:underline">
                        <a href="/manager/finalrequest/details/${finalRequest.unique_code}">
                    ...
                    <td class="py-2 px-3 text-sm text-gray-700">${finalRequest.current_process_index || "N/A"}/${finalRequest.total_processes || "N/A"}</td>
                    ...
                    <td class="py-2 px-3 text-sm text-center">
                        ${getStatusIcon(finalRequest.manager_{{ Auth::guard('manager')->user()->manager_number }}_status)}
                    </td>
                </tr>
            `;
        ...
        channel.bind("status-updated", function (data) {
            ...
            let row = document.querySelector(`#finalrequest-row-${finalRequest.unique_code}`);

            if (row) {
                // Update the status icon for the specific manager
                let managerNumber = {{ Auth::guard('manager')->user()->manager_number }};
                let status = finalRequest[`manager_${managerNumber}_status`];
        ...
        function updateRowNumbers() {
            let rows = document.querySelectorAll("#finalrequests-table-body tr");
